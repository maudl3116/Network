...
where $Z(\theta)$ is a normalisation constant satisfying $Z(\theta)=\sum_{a \in \mathcal{A}_n}\exp\left[\theta^tT(a)\right]$ and $\theta,T(a) \in \mathbb{R}^m$ represent the vector of parameters and the vector of sufficient statistics respectively. The 1-dimensional case with $\theta = \frac{\rho}{1-\rho}$ gives us the expression for the Bernoulli graph. \
